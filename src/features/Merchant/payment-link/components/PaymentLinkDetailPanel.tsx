      ...
      <div className="mt-4 space-y-3">
        <PanelCard className="space-y-4 pb-3.5 pt-2.5">
          <InfoRow
            label="Status"
            value={
              <span className={link.status === 'active' ? 'text-brand-completed' : 'text-brand-muted'}>
                {link.status === 'active' ? 'Active' : link.status === 'paused' ? 'Paused' : 'Completed'}
              </span>
            }
          />
        ...
        <PanelCard className="py-2.5">
          ...
          <div className="mt-2 flex items-center gap-2.5">
            <span className="flex size-11.5 items-center justify-center rounded-full bg-brand-primary font-orbitron-bold text-2xl font-bold text-brand-white">
              M
            </span>
            <div>
              <p className="text-fig-14 font-urbanist-semibold text-brand-primary">Main Account</p>
              <p className="text-fig-10 text-brand-muted">
                Metro business <span className="mx-1">{"\u2022"}</span>
                <span className="font-urbanist-light text-brand-primary">@metrobusiness</span>
              </p>
            </div>
          </div>
        </PanelCard>
        ...
        <PanelCard>
          ...
          <div className="mb-3 flex items-center justify-between gap-2 rounded-xl bg-brand-white px-3 py-2.5">
            ...
            <button
              ...
              className="shrink-0 text-brand-blue cursor-pointer"
            >
        ...
        <PanelCard>
          <div className="mb-3 flex items-center justify-between">
            ...
            <p className="text-fig-12 font-urbanist-medium text-brand-muted">
              {remaining} orders left · {link.used}/{link.limit}
            </p>
          </div>
          <div className="h-2 overflow-hidden rounded-full bg-brand-track">
            <div className="h-full rounded-full bg-brand-ink" style={{ width: `${progress}%` }} />
          </div>
          <p className="mt-3 text-fig-10 text-brand-muted">
            Payment link will be marked complete once all orders are paid
          </p>
        </PanelCard>

        {link.payers.length > 0 && (
          <PanelCard>
            <p className="mb-3 text-fig-12 font-urbanist-semibold text-brand-primary">Paid by</p>
            <ul className="space-y-3">
              {link.payers.map((payer) => (
                <li key={payer.id} className="flex items-center justify-between gap-3">
                  <span className="flex min-w-0 items-center gap-2.5">
                    <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-brand-primary text-fig-10 font-urbanist-semibold text-brand-white">
                      {initials(payer.name)}
                    </span>
                    <span className="min-w-0">
                      <span className="block truncate text-fig-12 font-urbanist-semibold text-brand-primary">
                        {payer.name}
                      </span>
                      <span className="block text-fig-10 text-brand-muted">Paid · {payer.paidAt}</span>
                    </span>
                  </span>
                  <button
                    type="button"
                    className="flex shrink-0 items-center gap-1 text-fig-10 font-urbanist-medium text-brand-muted cursor-pointer"
                    onClick={() => void navigator.clipboard?.writeText(payer.reference)}
                  >
                    {payer.reference}
...
function ActionChip({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <button
      type="button"
      className="flex flex-1 items-center justify-center gap-2 rounded-full border border-brand-border bg-brand-white px-3 py-2.5 text-fig-12 font-urbanist-semibold text-brand-primary transition hover:bg-brand-surface cursor-pointer"
    >
      {icon}
      {label}
    </button>
...
function InfoRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3 text-fig-14">
      <span className="text-brand-muted">{label}</span>
      <span className="font-urbanist-medium text-brand-primary">{value}</span>
    </div>
  )
}
...
function ShareAction({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <button
      type="button"
      className="flex w-full items-center gap-3 py-2 text-fig-14 font-urbanist-medium text-brand-primary cursor-pointer"
    >
      {icon}
      {label}
    </button>
