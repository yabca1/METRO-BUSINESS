...
export function QrCodeDetailPanel({
  ...
  const statusLabel =
    qr.status === 'active' ? 'Active' : qr.status === 'paused' ? 'Paused' : 'Completed'
  const statusClass =
    qr.status === 'active' ? 'text-brand-completed' : 'text-brand-muted'
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 px-1">
        ...
        {isDynamic && qr.amount != null && (
          <p className="mt-1 text-fig-14 font-poppins-semibold text-brand-primary">
      ...
      <div className="mt-4 space-y-3">
        ...
        <PanelCard>
          ...
          <div className="mb-3 flex items-center justify-between gap-2 rounded-xl bg-brand-white px-3 py-2.5">
            <p className="truncate text-fig-12 font-urbanist-medium text-brand-blue">
              {truncateUrl(qr.url)}
            </p>
            <button
              type="button"
...
function QrPreviewCard({ handle }: { handle: string }) {
  return (
    <div className="overflow-hidden rounded-xl border border-brand-border bg-brand-white">
      <div className="bg-brand-primary px-4 py-3 text-center">
        <p className="font-orbitron-bold text-[16px] leading-none tracking-wide text-transparent bg-clip-text bg-gradient-to-l from-brand-indicator to-brand-secondary">
          Metro
          <span className="ml-1 text-[10px] tracking-[0.09em]">Birr</span>
        </p>
      </div>
      <div className="flex flex-col items-center px-6 py-6">
        <div className="relative flex h-48 w-48 items-center justify-center rounded-lg bg-brand-white p-3 shadow-sm ring-1 ring-brand-border">
          <Image src="/qr.png" alt="QR code" width={176} height={176} className="rounded-lg object-contain" />
        </div>
        <span className="mt-4 rounded-full border border-brand-border bg-brand-surface/60 px-3 py-1 text-fig-10 font-poppins-semibold text-brand-primary">
          {handle}
        </span>
      </div>
    </div>
